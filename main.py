...
def request_password_reset(payload: PasswordResetRequest):
    ...
    if user:
        ...
        send_email(
            email=payload.email, subject=subject, content=content, type="password_reset"
        )
...
def confirm_password_reset(payload: PasswordResetConfirm):
    ...
    db["users"].update_one(
        {"id": reset["user_id"]},
        {"$set": {"hashed_password": hashed, "updated_at": datetime.now()}},
    )
    # Mark token used
...
async def upload_document(
    ...
    utils.logger.info(
        f"Document upload request received for user: {user_id}, filename: {file.filename}"
    )
    ...
    try:
        ...
        utils.logger.error(
            f"HTTPException during document upload for user: {user_id}, file: {file.filename}"
        )
        raise
    except Exception as e:
        utils.logger.error(
            f"Unexpected error during document upload for user: {user_id}, file: {file.filename}: {str(e)}",
            exc_info=True,
        )
        raise HTTPException(
...
async def search_documents(
    ...
    user_id = current_user.id
    utils.logger.info(
        f"Search request received - Query: '{query}', User: {user_id}, Collection: {collection_name}, Results: {n_results}"
    )

    try:
        if collection_name:
            # Search in specific collection
            results = utils.search_in_collection(
                query, collection_name, user_id, n_results
            )
            ...
            top_results, collections_count = utils.search_across_user_collections(
                query, user_id, n_results
            )
        ...
        utils.logger.error(
            f"HTTPException during search for user: {user_id}, query: '{query}'"
        )
        raise
    except Exception as e:
        utils.logger.error(
            f"Error searching documents for user: {user_id}, query: '{query}': {str(e)}",
            exc_info=True,
        )
        raise HTTPException(
...
async def list_collections(
    page: int = 1, limit: int = 20, current_user=Depends(get_current_user)
):
    user_id = current_user.id
    ...
    try:
        ...
        utils.logger.info(
            f"List collections completed successfully: {len(collection_info)} collections returned"
        )
        return response_data
        ...
        utils.logger.error(
            f"Error listing collections for user: {user_id}: {str(e)}", exc_info=True
        )
        raise HTTPException(
...
async def delete_collection(
    collection_name: str, current_user=Depends(get_current_user)
):
    user_id = current_user.id
    utils.logger.info(
        f"Delete collection request received - Collection: {collection_name}, User: {user_id}"
    )
    ...
    try:
        ...
        utils.logger.error(
            f"HTTPException during delete collection - Collection: {collection_name}, User: {user_id}"
        )
        raise
    except Exception as e:
        utils.logger.error(
            f"Error deleting collection {collection_name} for user {user_id}: {str(e)}",
            exc_info=True,
        )
        raise HTTPException(
...
@app.get("/ai/ask")
def ask_ai(
    query: str, collection_name: str | None, current_user=Depends(get_current_user)
):
    try:
        if not utils.check_if_llm_keys_exists():
            return JSONResponse(content="Cannot invoke LLM. Please try again later!")
        response = rag_ai.invoke(
            {
...
def get_chat_history(
    ...
    try:
        ...
        items = list(
            db["user_chat"].find(query_filter).sort("created_at", -1).limit(limit)
        )
            ...
            messages.append(
                {
                    ...
                    "is_feedback_submitted": it.get("is_feedback_submitted"),
                    "created_at": it.get("created_at").isoformat(),
                }
            )
...
def feedback(
    payload: FeedbackRequest = Body(...), current_user=Depends(get_current_user)
):
    try:
        user_id = current_user.id
        query = payload.query if payload.query else None
        chat_id = payload.chat_id if payload.chat_id else None
        is_positive_feedback = (
            payload.is_positive_feedback if payload.is_positive_feedback else None
        )
        comments = payload.comments if payload.comments else None

        target_chat = utils.get_latest_or_previous_chat(user_id, query, chat_id)
        if not target_chat:
        ...
        target_chat_id = target_chat["_id"]
        utils.logger.info(f"target chat id {target_chat_id}")
        db["user_chat"].update_one(
        {"_id": target_chat_id},
        {"$set": {"is_feedback_submitted": True}}
        )
