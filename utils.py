...
def process_pdf_file(file_path: str, filename: str, user_id: str):
    ...
    try:
        ...
        logger.info(
            f"Vector store created successfully for collection: {collection_name}"
        )
        ...
        pdf_summary_obj = DocSummary(
            user_id=user_id,
            filename=filename,
            collection_name=collection_name,
            summary=llm_response.content,
        )
        summary_saved_response = db["doc_summary"].insert_one(
            pdf_summary_obj.model_dump()
        )
        logger.info(
            f"{filename} Summary saved in DB successfully: {summary_saved_response}"
        )
        result = {
...
def search_in_collection(
    query: str, collection_name: str, user_id: str, n_results: int = 5
):
    """
    ...
    if not collection_name.startswith(f"user_{user_id}_"):
        logger.warning(
            f"Access denied: User {user_id} attempted to access collection {collection_name}"
        )
...
@tool()
def search_across_user_collections(
    query: str, user_id: str, n_results: int = 5, state_context: dict = None
) -> tuple[list, int]:
    """
    ...
    user_collections = [
        col for col in collections if col.name.startswith(f"user_{user_id}_")
    ]
    ...
    logger.info(
        f"Cross-collection search completed: {len(top_results)} results returned from {len(user_collections)} collections"
    )
...
def get_user_collections(user_id: str):
    ...
    for collection in user_collections:
        ...
        logger.debug(
            f"Collection: {collection.name}, Document: {doc_name}, Count: {count}"
        )

        collection_info.append(
            {"name": collection.name, "document_name": doc_name, "count": count}
        )
...
def delete_user_collection(collection_name: str, user_id: str):
    ...
    if not collection_name.startswith(f"user_{user_id}_"):
        logger.warning(
            f"Access denied: User {user_id} attempted to delete collection {collection_name}"
        )
        raise HTTPException(status_code=403, detail="Access denied to this collection")
...
def validate_pdf_file(filename: str, file_size: int = None):
    ...
    if file_size and file_size > 10 * 1024 * 1024:
        logger.warning(
            f"Upload request rejected: File too large: {filename}, size: {file_size}"
        )
        raise HTTPException(
...
def cleanup_temp_file(temp_file_path: str):
    ...
    if temp_file_path and os.path.exists(temp_file_path):
        try:
            ...
            logger.error(
                f"Error cleaning up temporary file {temp_file_path}: {str(cleanup_error)}"
            )
            ...
            with open(filename, "w") as log_file:
                for message in messages:
                    if (
                        ...
                        and message.content.strip() != "exit"
                    ):
...
def load_chat_history(user_id: str, collection_name: str | None, save_to_text_file: bool = False):
    """
    ...
    try:
        ...
        logger.info(
            f"No previous conversation log found for {user_id}. Starting fresh."
        )
    except Exception as e:
...
def get_latest_or_previous_chat(user_id: str, query: Optional[str] = None):
    ...
    if user_id and query:
        ...
        chats = (
            db["user_chat"].find({"user_id": user_id}).sort("created_at", DESCENDING)
        )
        chats = list(chats)
        return chats[0] if chats else None
    else:
